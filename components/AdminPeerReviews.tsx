...
export const AdminPeerReviews: React.FC<AdminPeerReviewsProps> = ({ agencies }) => {
    ...
    const { reviews: globalReviews } = useGame(); // REVIEWS FROM CONTEXT
    
    // Le mode archive est maintenant géré par filtre de date ou de champ "archived" si on en rajoute un.
    // Pour l'instant, on considère que tout est dans la collection 'reviews'.
    // On peut imaginer qu'une review > 1 semaine est une archive.
