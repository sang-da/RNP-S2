...
import { BilanValidationModal } from './BilanValidationModal';
import { BilanSimulation } from '../../../../types';
...
export const ControlPanelModal: React.FC<ControlPanelModalProps> = ({ isOpen, onClose }) => {
    const { gameConfig, updateGameConfig, processFinance, processPerformance, simulatePerformance } = useGame();
    const [isValidationOpen, setIsValidationOpen] = useState(false);
    const [simulation, setSimulation] = useState<BilanSimulation | null>(null);
    ...
    const handleOpenValidation = () => {
        const sim = simulatePerformance('ALL');
        setSimulation(sim);
        setIsValidationOpen(true);
    };

    const handleConfirmPerformance = async (adjustments: { [agencyId: string]: number }) => {
        setIsProcessing(true);
        try {
            await processPerformance('ALL', adjustments);
    ...
    return (
        <>
            ...
            <BilanValidationModal 
                isOpen={isValidationOpen}
                onClose={() => setIsValidationOpen(false)}
                simulation={simulation}
                onConfirm={handleConfirmPerformance}
